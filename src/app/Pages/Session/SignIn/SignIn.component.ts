...
export class SignInComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.accountService.login(userlogin.Username, userlogin.Password).subscribe(
      result => {
        ...
        this.accountService.rememberMyUserName(this.insertForm.get('Username').value, userlogin.RememberMe);
        ...
        if (error.status == 400) {
          ...
          for (var i = 0; i < error.error.errors['Password'].length; i++) {
            if (error.error.errors['Password'][i] == "The field Password must match the regular expression '^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9_]).*$'.")
              this.messagesList.push('"The field Password must contain at least 6 characters, including at least 1 Lower case, 1 Upper case, 1 Digits and 1 Special character');
            console.log(error.error.errors['Password'][i]);
          }
        }

        else if (error.status == 401) this.messagesList.push(error.error.loginError);
  ...
  ngOnInit() {
    ...
    if (rememberMeStatus) this.Username.setValue(this.accountService.getRememberMeUserName());
